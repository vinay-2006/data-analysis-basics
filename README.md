...
🧾 Short summary
-------------
A collection of structured, reproducible notebooks and exported artifacts that demonstrate practical patterns for data ingestion, preprocessing, exploratory data analysis (EDA), and visualization using Python, NumPy, and Pandas. Materials are presented as implementation-focused notebooks and serialized artifacts for reference and reuse.
...
- 🧩 Emphasizes explicit data lineage and clear processing steps; implementations and rigor may vary by notebook.
...
------------------------------
Typical notebooks address themes such as data ingestion and basic checks, dtype management and cleaning strategies, exploratory data analysis and group summaries, and visualization and figure export. Files and naming vary; expect goal-oriented notebooks that demonstrate these themes and patterns.
...
🏗 Design & architecture
---------------------
The repository presents a conceptual data flow as a mental model: raw ingestion → cleaning & type handling → aggregated analysis → visual export. This description is intended to guide notebook organization and data handling; implementations may vary by notebook.
...
- 🔹 Use Parquet or Feather for intermediate storage when file size or I/O behavior makes it appropriate.  
...
- Index semantics and alignment rules; prefer vectorized alternatives over row-wise operations
...
----------------------
- 🔹 Checkpoint processed data only when it represents a stable, canonical artifact; otherwise exclude generated files via `.gitignore`.  
